...
export async function onLoad(ctx: PluginContext): Promise<void> {
  ctx.logger.info(`[cortex-plugin-stripe] Loaded`);
...
const stripeCreateCheckoutTool: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!customerId) {
        return {
          toolName: 'stripe_create_checkout',
          success: false,
          output: '',
          error: 'customer_id is required',
          durationMs: Date.now() - start,
      ...
      if (!result.ok) {
        return {
          ...
          success: false,
          output: '',
          error: result.error || 'Checkout creation failed',
          durationMs: Date.now() - start,
        };
      }
      return {
  ...
  definition: {
    ...
    params: [
      ...
      },
      {
        ...
        required: false,
        defaultValue: 'active',
        options: ['active', 'past_due', 'canceled', 'all'],
      },
    ],
    capabilities: ['network:fetch'],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!['daily', 'weekly', 'monthly', 'yearly'].includes(period)) {
        return {
          toolName: 'stripe_revenue_report',
          success: false,
          output: '',
      ...
      if (!result.ok) {
        return {
          ...
          success: false,
          output: '',
          error: result.error || 'Report failed',
          durationMs: Date.now() - start,
        };
      }
      ...
      const totalAmount = transactions.reduce((sum: number, t: Record<string, unknown>) =>
        sum + (Number(t.amount) || 0), 0) / 100;
      const totalFees = transactions.reduce((sum: number, t: Record<string, unknown>) =>
        sum + (Number(t.fee) || 0), 0) / 100;
      const report = {
        period,
